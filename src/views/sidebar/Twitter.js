...
const Twitter = props => {
  ...
  let liveTweetsRef = useRef(null),
      liveTimeLinesRef = useRef(null)

  useEffect(() => {
    if (liveTweetsRef.current == null) {
      liveTweetsRef.current = React.firebase.firebase.database(React.firebase.twitter).ref(`tweets`).orderByKey().limitToLast(100)
      liveTimeLinesRef.current = React.firebase.firebase.database(React.firebase.twitter).ref(`timelines`)
    }
  }, [])
  ...
  function listenForTweets() {
    liveTweetsRef.current.on('child_added', (val) => {
  ...
  useEffect(() => {
    const ref = liveTimeLinesRef.current.on('value', (val) => {
        setTimelines(
  ...
  useEffect(() => {
    return () => {            
      liveTweetsRef.current.off('child_added')
      liveTimeLinesRef.current.off('value')
    }
}, [])
