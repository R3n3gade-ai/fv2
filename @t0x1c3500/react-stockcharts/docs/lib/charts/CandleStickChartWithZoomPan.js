...
class CandleStickChartWithZoomPan extends React.Component {
	...
	componentWillMount() {
		...
		window.addEventListener('mousewheel', this.onKeyPress, { passive: false })
	}
	componentWillUnmount() {
		window.removeEventListener('mousewheel', this.onKeyPress, { passive: false })
	}
	saveNode(node) {
		this.node = node;
	}
	...
	onKeyPress(event) {
		console.log('mousewheeel bro')
		if (event.ctrlKey) {
			console.log('preventing bro')
