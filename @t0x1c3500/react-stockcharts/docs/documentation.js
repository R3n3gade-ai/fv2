...
const INDICATORS = {
	...
	pages: [
		...
		require("./lib/page/FlowTradePage").default,
	]
};
...
function parseData(parse, parseIt = true) {
	return function(d) {
		if (parseIt) {
			d.date = parse(d.date);
		} else {
...
function loadPage() {
	...
	const promisegroupedBarData = fetch("data/groupedBarData.json")
		.then(response => response.json());
	const promiseFlowtradeData = fetch("data/flowtrade.tsv")
		.then(response => response.text())
		.then(data => tsvParse(data, parseData(parseDate, false)));
	const promiseBlocksData = fetch("data/blocksData.json")
		.then(response => response.json());

	Promise.all([promiseMSFT, promiseMSFTfull, promiseIntraDayContinuous, promiseIntraDayDiscontinuous, promiseCompare, promiseBubbleData, promiseBarData, promisegroupedBarData, promiseFlowtradeData, promiseBlocksData])
		.then(function(values) {
			const [MSFT, MSFTfull, intraDayContinuous, intraDayDiscontinuous, compareData, bubbleData, barData, groupedBarData, flowTradeData, blocksData] = values;
			...
			renderPage(MSFT, MSFTfull, intraDayContinuous, intraDayDiscontinuous, compareData, bubbleData, barData, groupedBarData, horizontalBarData, horizontalGroupedBarData, flowTradeData, blocksData);
...
function renderPage(data, dataFull, intraDayContinuous, intraDayDiscontinuous, compareData, bubbleData, barData, groupedBarData, horizontalBarData, horizontalGroupedBarData, flowTradeData, blocksData) {
