...
export default function financeDiscontinuousScale(
	...
	scale.ticks = function(m, flexTicks, levels) {
		...
		if (levels !== null) {
			levels.map(level => {
				unsortedTicks = unsortedTicks.concat(ticksMap.get(level));
			});
		}

		const uniqueTicks = [...new Set(unsortedTicks)];
		const ticks = uniqueTicks.sort(ascending);
