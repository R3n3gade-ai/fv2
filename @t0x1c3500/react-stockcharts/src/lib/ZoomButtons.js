...
class ZoomButtons extends Component {
	...
	render() {
		const { chartConfig, xScale } = this.context;
		...
		const { onReset, onFirst, dataLength } = this.props;
		const showFirst = xScale.domain()[1] < ( dataLength - 1 );
		...
		const firstX = centerX + 2.075 * w + 2 * strokeWidth;

		return (
			<g className="react-stockcharts-zoom-button_container">
				<rect className="react-stockcharts-zoom-fade-in"
					x={zoomOutX - ( 7 * w ) / 4}
					y={y}
					rx={rx}
					ry={ry}
				...
				<g className="react-stockcharts-zoom-button">
					...
					<React.Fragment>
						<rect
							...
							strokeWidth={strokeWidth}
						/>
						<g transform={`translate (${resetX + 2 * w}, ${y + h / 4}) scale(.45)`}>
					...
					<rect className="react-stockcharts-enable-interaction in"
						...
						fill="none"
					/>
					{showFirst && <rect className="react-stockcharts-enable-interaction first"
						onClick={onFirst}
						x={firstX - w / 2}
						y={y}
						rx={rx}
