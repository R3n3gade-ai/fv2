...
class ChartCanvas extends Component {
	...
	handleZoom(zoomDirection, mouseXY, e) {
		...
		const { fullData } = this;
		const editedZoomAnchor = e.ctrlKey ? mouseBasedZoomAnchor : rightDomainBasedZoomAnchor;
		const item = editedZoomAnchor({
			xScale: initialXScale,
			xAccessor,
			mouseXY,
			plotData: initialPlotData,
		...
		localStorage.setItem(this.props.chartUid, JSON.stringify(xScale.domain()));
		localStorage.setItem("scale_" + this.props.chartUid, JSON.stringify({
			"length": xScale.domain()[1] - xScale.domain()[0],
			"end": xScale.domain()[1] - fullData.length
		}));
