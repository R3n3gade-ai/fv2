...
import { first, last, hexToRGBA, isNotDefined, isDefined, identity, zipper, strokeDashTypes, getStrokeDasharray, isObject } from "../utils";
...
function tickHelper(props, scale) {
	...
	if (isDefined(tickValuesProp)) {
		...
		tickValues = scale.ticks(tickArguments, true, customLevels);
	} else {
	...
	if (orient === "bottom" || orient === "top") {
		...
		ticks = tickValues.map(tickDValue => {
			const d = isObject(tickDValue) ? tickDValue.index : tickDValue;
